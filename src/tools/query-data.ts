...
interface QueryEnv {
    ALLELE_REGISTRY_DATA_DO?: unknown;
}

export function registerQueryData(server: McpServer, env?: QueryEnv): void {
    const handler = createQueryDataHandler("ALLELE_REGISTRY_DATA_DO", "allele_registry");

    server.registerTool(
        "allele_registry_query_data",
        {
            title: "Query Staged Allele Registry Data",
            description:
                "Query staged ClinGen Allele Registry data using SQL. Use this when responses are too large and have been staged with a data_access_id.",
            inputSchema: {
