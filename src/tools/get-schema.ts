...
export function registerGetSchema(server: McpServer, env?: SchemaEnv): void {
    const handler = createGetSchemaHandler("ALLELE_REGISTRY_DATA_DO", "allele_registry");

    server.registerTool(
        "allele_registry_get_schema",
        {
            title: "Get Staged Data Schema",
            description:
                "Get schema information for staged ClinGen Allele Registry data. Shows table structures and row counts. " +
